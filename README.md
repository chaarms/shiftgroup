# Shift Group LLC — Website

A modern, single-page site for Shift Group LLC, a government contracting startup offering procurement, staffing, consulting, IT support, and cybersecurity services.
...
1. Create a new repository (e.g. `shift-group-site`) and push these files to the `main` branch — keep `index.html` at the repo root.
2. In the repo, go to **Settings → Pages**.
3. Under **Build and deployment**, set **Source** to `Deploy from a branch`.
4. Set **Branch** to `main` and folder to `/ (root)`, then **Save**.
5. GitHub gives you a URL like `https://<username>.github.io/shift-group-site/` within a minute or two.
6. Optional: add a `CNAME` file with your custom domain (e.g. `shiftgroupllc.com`) if you buy one, and point its DNS at GitHub Pages.

## Things to replace before launch

Search the files for these placeholders:

- **Contact info** — `contact@shiftgroupllc.com` and the phone number in `index.html` (Contact section and footer `mailto:`/`tel:` links).
